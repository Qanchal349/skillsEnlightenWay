...
import APIFeatures from "../../backend/utils/features.js"

export const getAllCourses = catchAsyncError(async(req,res,next)=>{


     const apiFeatures = new APIFeatures(Course,req.query).search();
     const courses = await apiFeatures.query.select("-lectures");
     res.status(200).json({
         success:true,
         courses
     })
...
export const createCourse = catchAsyncError(async(req,res,next)=>{
     ...
     if(!title || !description || !category || !createdBy)
       return next(new ErrorHandler("Please fill required data",400))
...
// add lectures to a course max size 100mb
...
export const deleteLecture = catchAsyncError(async(req,res,next)=>{
  ...
  const lecture = course.lectures.find((item)=>{
     if(item.id.toString()===lectureId.toString()) return item;
  })
